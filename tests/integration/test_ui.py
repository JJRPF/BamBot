...
@pytest.mark.asyncio
async def test_custom_ui_e2e(ui_test_server):
    async with async_playwright() as p:
        ...
        # Simulate typing a chat message asking for printer status
        chat_input = page.locator("#chat-input")
        await chat_input.fill("what is the printer status?")
        await chat_input.press("Enter")
        logger.info("Sent chat query.")

        # Wait for the agent's bubble response to appear
        agent_bubble = page.locator(".chat-msg.agent .bubble").last
        await agent_bubble.wait_for(state="visible", timeout=30000)

        # Assert no raw JSON and that it contains Printer Status or State
        text = await agent_bubble.inner_text()
        logger.info("Agent response text: %s", text)
        assert len(text) > 0, "Agent response was empty"
        assert "{" not in text, f"Agent response contains raw planning JSON: {text}"
        assert "printer status" in text.lower() or "state" in text.lower(), f"Expected printer status message, got: {text}"
